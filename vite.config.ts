...
import { AutonomousAgentEngine } from './supabase/functions/_shared/autonomous-agent/engine.ts'
      ...
      tsconfigPaths(),
      {
        ...
        configureServer(server) {
          server.middlewares.use(async (req, res, next) => {
            ...
            if (req.url === '/api/wake-all' && req.method === 'POST') {
              try {
                console.log('--- LOCAL BULK WAKE TRIGGERED ---');
                ...
                const results = await Promise.allSettled((agents || []).map(async (agent) => {
                  const engine = new AutonomousAgentEngine(
                    ...
                    env.ENCRYPTION_KEY || env.VITE_ENCRYPTION_KEY
                  );
                  return await engine.wakeAgent(agent.id, true);
                }));

                res.statusCode = 200;
                res.setHeader('Content-Type', 'application/json');
                res.end(JSON.stringify({
                  message: `Bulk wake complete for ${(agents || []).length} agents`,
                  results: results.map(r => r.status === 'fulfilled' ? r.value : { status: 'error', error: 'Internal failure' })
                }));
              } catch (error: any) {
                console.error('Bulk wake failed:', error);
                res.statusCode = 500;
            ...
            if (req.url?.includes('list-models')) {
              ...
              // Use a separate async block to keep middleware synchronous
              (async () => {
                try {
